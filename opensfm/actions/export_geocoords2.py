...
def run_dataset(
    ...
    if not (transformation or image_positions or reconstruction or dense or georef):
    ...
    if georef:
        reconstructions = data.load_reconstruction()
        for r in reconstructions:
            id, eo, pts = _georef_reconstruction(r, t)
...
def _georef_reconstruction(
    ...
    i = 0
    id = []
    eo = np.empty(shape=(len(reconstruction.shots), 3+9))
    for shot in reconstruction.shots.values():
        R = shot.pose.get_rotation_matrix()
        shot.pose.set_rotation_matrix(np.dot(R, A1))
        shot.pose.set_origin(np.dot(A, shot.pose.get_origin()) + b)
        ...
        ori = shot.pose.get_rotation_matrix()
        eo[i, :3] = pos
        eo[i, 3:] = ori.ravel()
        i += 1
        
        print(f'{id[-1]}\t{pos[0]:.3f}\t{pos[1]:.3f}\t{pos[2]:.3f}')
        
    j = 0
    pts = np.empty(shape=(len(reconstruction.points), 3+3))
    for point in reconstruction.points.values():
        point.coordinates = list(np.dot(A, point.coordinates) + b)
        color = point.color / 255
        pts[j, :3] = point.coordinates
        pts[j, 3:] = color
        j += 1
